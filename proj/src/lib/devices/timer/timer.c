...
int (timer_set_frequency)(uint8_t timer, uint32_t freq) {
  ...
  uint8_t less_sig_ticks, most_sig_ticks;
  util_get_LSB(ticks, &less_sig_ticks);
  util_get_MSB(ticks, &most_sig_ticks);
  ...
  switch (field) {
    ...
    case tsf_mode:
      ...
      if (BIT(1) & field_value.count_mode) {
        field_value.count_mode &= 0x3;  // make dont care bits 0 for future compatibility
      }
      break;
    case tsf_base:
